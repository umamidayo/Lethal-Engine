...
function BackpackScript.init()
	...
	local lastBackpackInteraction = tick()

	-- Topbar logic
	local BackpackEnabled: boolean = true

	local function GetIconModule(): ModuleScript
		local ReplicatedIconModule: ModuleScript = ReplicatedStorage:WaitForChild("Dependencies"):FindFirstChild("Icon")
		local LocalIconModule: ModuleScript = script:FindFirstChild("Icon")

		if ReplicatedIconModule and ReplicatedIconModule:IsA("ModuleScript") then
			if LocalIconModule then
	...
	-- InventoryIcon:setLabel("Backpack", "hovering")
	...
	local NumberOfInventoryRows = IsVR and INVENTORY_ROWS_VR
	...
	local changeToolFunc = function(actionName: string, inputState: Enum.UserInputState, inputObject: InputObject): ()
			...
			SlotFrame.DragStopped:Connect(function(x: number, y: number): ()
				...
				elseif CheckBounds(HotbarFrame, x, y) then
					...
					if tool and tool.CanBeDropped then
						...
						ReplicatedStorage.RemotesLegacy.DropTool:FireServer(tool)
					else
						if slot.Index <= NumberOfHotbarSlots then
	...
	local function unbindAllGamepadEquipActions(): ()
	...
	local function getGamepadSwapSlot(): any
		for i = 1, #Slots do
	...
	local function vrMoveSlotToInventory(): ()
	...
	local function disableGamepadInventoryControl(): ()
	...
	local function gamepadDisconnected(): ()
	...
	local function OnIconChanged(enabled: boolean): ()
		...
		enabled = enabled and pcall(StarterGui.GetCore, StarterGui, "TopbarEnabled")
	...
	ScrollDownInventoryButton =
		MakeVRRoundButton("ScrollDownButton", "rbxasset://textures/ui/Backpack/ScrollUpArrow.png")
	...
	local function resizeGamepadHintsFrame(): ()
		...
		for i = 1, #gamepadHints do
			...
			spaceTaken = spaceTaken
				+ (gamepadHints[i].HintText.Position.X.Offset + gamepadHints[i].HintText.TextBounds.X)
		end
	...
	local autohideBackpack = false

	Player:GetAttributeChangedSignal("HideUI"):Connect(function()
		autohideBackpack = Player:GetAttribute("HideUI")

		if not autohideBackpack then
	...
	RunService.Heartbeat:Connect(function(dt): ()
		...
		if tick() - lastBackpackInteraction > 2 then
