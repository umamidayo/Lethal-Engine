...
local function initializeSoundSystem(player: Player, humanoid: Humanoid, rootPart: BasePart)
	...
	floorRayParams.FilterDescendantsInstances = { player.Character, workspace.Forcefields, workspace.Landscape }
	local floorRayResult: RaycastResult, SoundInfo: Sound
	...
	local loopedSoundUpdaters = {
		...
		[sounds.Running] = function(_, sound, vel)
			floorRayResult = workspace:Raycast(rootPart.Position, -Vector3.yAxis * 5, floorRayParams)
			if not floorRayResult then
				return
			end

			if not SoundInfo or floorRayResult.Material.Name ~= SoundInfo.Name then
				SoundInfo = FootstepsSoundGroup:FindFirstChild(floorRayResult.Material.Name)
			end

			if not SoundInfo then
				SoundInfo = FootstepsSoundGroup["nil Sound"]
				sound.SoundId = SoundInfo.SoundId
				sound.PlaybackSpeed = SoundInfo.PlaybackSpeed
				sound.Volume = SoundInfo.Volume
			else
				sound.SoundId = SoundInfo.SoundId
				sound.PlaybackSpeed = SoundInfo.PlaybackSpeed * (vel.Magnitude / 20)
				sound.Volume = SoundInfo.Volume * (vel.Magnitude / 12)
			end
		end,
	}
	...
	local steppedConn = RunService.Heartbeat:Connect(function(worldDt)
		elapsed += worldDt
		if elapsed < 0.1 then
			return
		end
		elapsed = 0
		if not rootPart or not LocalPlayer.Character then
			return
		end
...
local function playerAdded(player: Player)
	local function characterAdded(character: Model)
		...
		while character:IsDescendantOf(workspace) and not humanoid do
		...
		while character:IsDescendantOf(workspace) and not rootPart do
			waitForFirst(
...
Humanoid.Running:Connect(function(a)
	vel = a
	RunningSound.PlaybackSpeed = FootstepsSoundGroup[Humanoid.FloorMaterial.Name].PlaybackSpeed
		* (a / 20)
		* (math.random(30, 50) / 40)
	RunningSound.Volume = FootstepsSoundGroup[Humanoid.FloorMaterial.Name].Volume * (vel / 12)
