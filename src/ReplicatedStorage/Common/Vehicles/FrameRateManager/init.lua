...
function FrameRateManager.getFps(self: Types.FrameRateManager)
	...
	for index = #self.frameUpdateTable, 1, -1 do
		self.frameUpdateTable[index + 1] = self.frameUpdateTable[index] >= self.lastIteration - 1
				and self.frameUpdateTable[index]
			or nil
	end

	self.frameUpdateTable[1] = self.lastIteration

	local fps = os.clock() - self.start >= 1 and #self.frameUpdateTable
		or #self.frameUpdateTable / (os.clock() - self.start)
