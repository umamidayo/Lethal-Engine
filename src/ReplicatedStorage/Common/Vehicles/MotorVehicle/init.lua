...
function MotorVehicle.new(config: Types.Config)
	...
	self._maxEngineRPM = config.maxRPM or 2000
	...
	self._minEngineRPM = config.minRPM or 500
	...
	self._throttleAcceleration = config.throttleAcceleration or 0.5
	...
	self.maxAngularAcceleration = config.maxAngularAcceleration or 25
...
function MotorVehicle._calcThrottle(self: Types.MotorVehicle, deltaTime: number)
	...
	local assemblyDirection = self:getAssemblyDirection()
	local chassis = if self.root.Parent then self.root.Parent else self.root
	...
	local tailLight = chassis:FindFirstChild("Lights").Tail.PointLight
	local reverseLight = chassis:FindFirstChild("Lights").Reverse.PointLight

	if assemblyVelocityState == Enums.AssemblyVelocityState.Breaking then
		Network.fireServer(Network.RemoteEvents.VehicleEvent, "brake", tailLight)
	elseif assemblyVelocityState == Enums.AssemblyVelocityState.Neutral then
		if self.tailLightsOn == false then
			Network.fireServer(Network.RemoteEvents.VehicleEvent, "neutralbrake", tailLight)
		end
	elseif assemblyVelocityState == Enums.AssemblyVelocityState.Accelerating then
		if assemblyDirection == -1 then
			if reverseLight.Enabled == false then
				reverseLight.Enabled = true
				Network.fireServer(Network.RemoteEvents.VehicleEvent, "reverse", reverseLight)
			end
		else
			if reverseLight.Enabled then
				reverseLight.Enabled = false
				Network.fireServer(Network.RemoteEvents.VehicleEvent, "reverseOff", reverseLight)
			end
	...
	if self._engineRPM > 3000 then
		for _, child in pairs(chassis.Parent.Model.Wheels:GetChildren()) do
			if child:FindFirstChild("Smoke") then
				child.Smoke.Enabled = true
			end
		end
	else
		for _, child in pairs(chassis.Parent.Model.Wheels:GetChildren()) do
...
function MotorVehicle.compute(
	self: Types.MotorVehicle,
	deltaTime: number,
