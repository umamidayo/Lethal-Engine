...
local MvUtil = require(ReplicatedStorage.Common.Vehicles.MotorVehicle.Util)
local MvEnums = require(ReplicatedStorage.Common.Vehicles.MotorVehicle.Enums)

local FrameRateManager = require(ReplicatedStorage.Common.Vehicles.FrameRateManager)
...
function Stats.compute(self: Types.Stats, info: Types.Info)
	...
	local direction = if info.chassisInst:getAssemblyVelocity() < 1
		then "None"
		else (info.chassisInst:getAssemblyDirection() > 0 and "Forward" or "Backwards")
	...
	self:_append(string.format("Frame Rate: %d fps", self.frameRateManager:getFps()))

	self:_append(string.format("Avg. Execution Time: %.2fμs", avgExecutionTime))

	self:_append(string.format(
		"Delta Time: %.2fms",
	...
	self:_append(string.format("Direction: %d (%s)", info.chassisInst:getAssemblyDirection(), direction))

	self:_append(string.format("State: %d (%s)", info.chassisInst:getAssemblyVelocityState(), state))

	self:_append(string.format("Velocity: %d (%dmph)", assemblyVelocity, Util.toMilesPerHour(assemblyVelocity)))
	...
	self:_append(string.format("Pitch: %.2f", info.chassisInst:getRevPlaybackSpeed()))

	self:_append(string.format("Torque: %d", info.chassisInst:getMotorTorque()))

	self:_append(string.format("Horsepower: %d", info.chassisInst:getMotorHorsepower()))

	self:_append(string.format("Angular Velocity: %s", Util.fmtInt(math.floor(info.output.angularVelocity))))

	self:_append(string.format("Angular Frequency: %drpm", math.floor(info.chassisInst:getMotorRPM())))

	self:_append(string.format("Max Angular Acceleration: %d", info.output.motorMaxAngularAcceleration))

	self:_append(string.format("Max Torque: %d", info.output.motorMaxTorque))

	self:_append("<b>Transmission</b>")

	self:_append(string.format("Gear: %s", info.chassisInst:getGear() .. MvUtil.getOrdinal(info.chassisInst:getGear())))

	self:_append("<b>Wheel</b>")

	self:_append(string.format("Turn Angle: %.2fdeg", info.output.angle))
end
...
function Stats.destroy(self: Types.Stats)
	...
	local template = self.gui.Stats.UIGridLayout.Template

	for _, child: Instance in self.gui.Stats:GetChildren() do
		if not child:IsA("UIGridLayout") and child ~= template then
			print(child.Name)
			child:Destroy()
		end
	end

	template.Parent = self.gui.Stats
end
